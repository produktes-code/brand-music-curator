...
def analyze_track(file_path, cutoff=80, target_bpm=None):
    try:
        ...
        result = {
            ...
            "key": key,
            "dsp_applied": f"High-pass Filter ({cutoff}Hz) - Retail Optimized"
            if cutoff > 0
            else "None",
        }
        logger.info(
            f"Análisis exitoso para {file_path}. BPM: {result['bpm']}, Key: {result['key']}"
        )
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Analizador de archivos de audio para DJ / Retail"
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=80,
        help="Frecuencia de corte del filtro paso-alto",
    )
