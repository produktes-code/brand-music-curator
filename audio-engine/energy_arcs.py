...
def generate_energy_arc(store_state, current_traffic_level):
    ...
    try:
        ...
        if store_state == "PRE_OPENING":
            ...
            target_arc["curation_logic"] = (
                "Motivación del staff. Ritmos alegres para tareas de limpieza y reposición."
            )
            ...
            target_arc["curation_logic"] = (
                "Alta energía para acelerar el flujo de salida de clientes y animar al personal en el cierre."
            )
            ...
            if traffic < 30:
                ...
                target_arc["curation_logic"] = (
                    "Afluencia baja: Reducción de BPMs para aumentar el tiempo de permanencia (Dwell Time) e impulsar ventas."
                )
            elif traffic > 80:
                target_arc["target_bpm_range"] = [120, 130]
                target_arc["target_energy"] = 85
                target_arc["curation_logic"] = (
                    "Afluencia alta: Incremento de BPMs para evitar aglomeraciones y estimular compras impulsivas."
                )
            else:
                target_arc["target_bpm_range"] = [105, 120]
                target_arc["target_energy"] = 60
                target_arc["curation_logic"] = (
                    "Afluencia media: Ritmo estándar de marca."
                )
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        print(
            json.dumps(
                {"status": "error", "message": "Missing store_state or traffic_level"}
            )
        )
        sys.exit(1)
